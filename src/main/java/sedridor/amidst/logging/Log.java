...
public class Log {
    ...
    public static void debug(Object... s) {
        if (!isShowingDebug) return;
        synchronized (logLock) {
            printWithTag("debug", s);
            if (listeners.size() != 0) for (LogListener listener : listeners.values()) listener.debug(s);
        }
    }
    ...
    public static void w(Object... s) {
        synchronized (logLock) {
            ...
            if (listeners.size() != 0) for (LogListener listener : listeners.values()) listener.warning(s);
        }
    }
    ...
    public static void e(Object... s) {
        synchronized (logLock) {
            ...
            if (isUsingAlerts) JOptionPane.showMessageDialog(null, s, "Error", 0);
            if (listeners.size() != 0) for (LogListener listener : listeners.values()) listener.error(s);
        }
    }
    ...
    public static void crash(Throwable e, String message) {
        synchronized (logLock) {
            printWithTag("crash", new Object[] { message });
            ...
            if (e != null) {
                ...
                printWithTag("crash", new Object[] { exceptionText });
            }
            if (listeners.size() != 0)
                for (LogListener listener : listeners.values()) listener.crash(e, exceptionText, message);
            new CrashDialog(message);
            if (FinderWindow.instance != null) FinderWindow.instance.dispose();
        }
    }
